...
# Find the spam messages the model is missing
X_test_reset = X_test.reset_index(drop=True)
y_test_reset = y_test.reset_index(drop=True)
predictions_series = pd.Series(predictions)

missed_spam = X_test_reset[(y_test_reset == 1) & (predictions_series == 0)]

print("\n=== Spam messages the model MISSED ===")
for msg in missed_spam:
    print("-", msg)